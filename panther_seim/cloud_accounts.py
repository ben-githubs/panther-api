...
from ._util import gql_from_file, UUID_REGEX, to_uuid, execute_gql, AWS_REGIONS, ARN_REGEX
...
class CloudAccountsInterface:
    ...
    def create(
        ...
        if resource_type_ignore is not None:
            ...
            #   benefit. Panther's API doesn't do this anyway, so there's no expectation that we would.
        ...
        try:
            result = execute_gql("cloud_accounts/create.gql", self.client, variable_values={"input": values})
            print(result)
    ...
    def delete(self, accountid: str) -> str:
        ...
        # Invoke API
        results = execute_gql("cloud_accounts/delete.gql", self.client, variable_values={"id": accountid})
        print(results)
